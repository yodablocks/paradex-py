...
TEST_L2_ADDRESS = int_from_hex(
    "0x4183e117a90be234f2a34efb66dbae17b879b05d6581f72c729197c592fb366"
)
TEST_L2_PRIVATE_KEY = int_from_hex(
    "0x713fd8c2c3bfb584601cd71b1012d26bf24497d313c9ad9d985cb8ac144096b"
)
TEST_L2_PUBLIC_KEY = int_from_hex(
    "0x2957ff4b29c5300f007865cd80eaaeb28a81352b22d2fb216845285004cbcd8"
)
...
def test_account_l1_private_key():
    ...
    print(f"Derived L2 Address: {hex(account.l2_address)}")
    print(f"Expected L2 Address: {hex(TEST_L2_ADDRESS)}")

    print(f"Derived L2 Private Key: {hex(account.l2_private_key)}")
    print(f"Expected L2 Private Key: {hex(TEST_L2_PRIVATE_KEY)}")
    print(f"Derived L2 Public Key: {hex(account.l2_public_key)}")
    print(f"Expected L2 Public Key: {hex(TEST_L2_PUBLIC_KEY)}")
